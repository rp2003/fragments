...
//     return new Fragment(data);  // Create a new Fragment instance
//   }
...
class Fragment {
  constructor({ id, ownerId, created, updated, type, size = 0 }) {
    ...
    if (!validTypes.includes(mime)) {
      throw new Error(`The requested '${mime}' MIME type is not supported yet.`);
    }
  ...
  async setData(data) {
    if (!Buffer.isBuffer(data)) throw new Error('supplied data is not Buffer');
    this.size = data.byteLength;
    await this.save();
    return writeFragmentData(this.ownerId, this.id, data);
  }
